...
def adicionar_produto(nome, categoria, preco, qtd):
    ...
    if nome in df['Nome'].values:
        ...
        # Gerar ID dinâmico baseado no maior ID existente para evitar duplicados
...
# ---- NOVA FUNÇÃO PEDIDA: ALTERAR OU EXCLUIR ESTOQUE ----
with aba_gerenciar:
if st.session_state.produtos.empty:
    ...
    # Buscar dados atuais do produto escolhido
    ...
    st.warning(f"Status Atual de **{prod_selecionado}**: {estoque_max := estoque_atual} unidades em estoque | Preço atual: R$ {preco_atual:.2f}")
    
    col_alt1, col_alt2 = st.columns(2)
    
    # Sub-bloco para Adicionar Mais Unidades (Ex: Tinha 3, comprou +10 = 13)
    with col_alt1.container(border=True):
        st.markdown("##### ➕ Entrada / Ajuste Positivo")
        qtd_mais = st.number_input("Adicionar quantas unidades ao estoque?", min_value=0, step=1, value=0)
        novo_preco = st.number_input("Alterar preço de venda para: (R$)", min_value=0.0, value=preco_atual, step=5.0)
    ...
    # Sub-bloco para Exclusão Definitiva
    with col_alt2.container(border=True):
        st.markdown("##### ❌ Exclusão de Mercadoria")
        st.write("Deseja retirar esse item permanentemente do seu catálogo?")
        confirmar_exclusao = st.checkbox("Sim, tenho certeza de que quero apagar este produto.")
        ...
        if st.button("Remover Produto do Sistema", type="primary"):
            if confirmar_exclusao:
                ...
                st.success(f"O produto '{prod_selecionado}' foi deletado do estoque.")
                st.rerun()
            else:
                st.error("Por favor, marque a caixinha de confirmação antes de excluir.")
...
aba1, aba2 = st.tabs(["📋 Histórico de Saídas (O que comprou)", "💵 Histórico de Entradas (Pagamentos feitos)"])
