...
if menu == "📊 Dashboard":
    ...
    # Cartões de métricas estilizados nativos
    ...
    if not st.session_state.produtos.empty:
        # Tabela profissional com barra de progresso visual para o estoque
        st.data_editor(
            st.session_state.produtos,
            column_config={
                "ID": st.column_config.NumberColumn("ID", width="small"),
                "Preço Venda": st.column_config.NumberColumn("Preço de Venda", format="R$ %.2f"),
                "Estoque Atual": st.column_config.ProgressColumn("Nível de Estoque", help="Quantidade disponível", min_value=0, max_value=50, format="%d un")
            },
            disabled=True,
...
# ================= RECURSO 2: GESTÃO DE ESTOQUE =================
    ...
    with aba_massa:
        st.info("💡 Dica profissional: Você pode copiar colunas inteiras do Excel e estruturar separando por vírgula.")
        st.markdown("""
...
```text
        ...
        if st.button("Processar e Salvar Lista"):
            if texto_colado.strip():
                ...
                for linha in linhas: # Bug fixado aqui de 'hashtags' para 'linhas'
                    try:
                ...
                if sucessos > 0:
                    st.success(f"Sucesso! {sucessos} produtos foram inseridos no estoque.")
                if erros > 0:
                    st.error(f"Aviso: {erros} linhas estavam fora do padrão e foram ignoradas.")
                if sucessos > 0:
                    st.rerun()
...
# ================= RECURSO 4: CLIENTES & CREDIÁRIO =================
    ...
    if not st.session_state.clientes:
        ...
        st.markdown("### 📋 Painel Geral de Contas (Formatado)")
        ...
        # Tabela Geral com formatação monetária bonita e limpa
        st.data_editor(
            df_geral_clientes,
            column_config={
        ...
        with st.container(border=True):
            st.markdown("#### 💰 Registrar Novo Abatimento (Entrada de Dinheiro)")
            if saldo_devedor > 0:
